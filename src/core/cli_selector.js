...
export class CLISelector {
    ...
    async selectCLI(options = {}) {
        // 環境変数で指定されている場合
        if (process.env.DEFAULT_AI_CLI) {
            const envCli = process.env.DEFAULT_AI_CLI.toLowerCase();
            if (['claude', 'gemini', 'codex'].includes(envCli)) {
                console.log(chalk.blue(`\n🤖 Using ${envCli} CLI (set by environment variable)\n`));
                this.updateLastUsed(envCli);
                return envCli;
            }
        }
