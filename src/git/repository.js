...
export class Repository {
    ...
    async getDiff(sourceBranch, targetBranch, filters = {}) {
        ...
        // Check if same branch comparison with time filter
        const isSameBranch = sourceBranch === targetBranch;
        let diffArgs;
        let baseCommitForSameBranch = null;
        
        if (isSameBranch && (filters.since || filters.until)) {
            // For same branch with time filter, get the commit before the time range
            const logArgs = ['rev-list', sourceBranch, '-n', '1'];
            
            // Get commit just before the time range
            if (filters.since) {
                logArgs.push(`--before=${filters.since}`);
            }
            
            try {
                const beforeCommit = await this.git.raw(logArgs);
                baseCommitForSameBranch = beforeCommit.trim().split('\n')[0]; // Take only first line
                
                if (baseCommitForSameBranch) {
                    // Compare from that commit to HEAD
                    diffArgs = [`${baseCommitForSameBranch}..HEAD`];
                } else {
                    // No commits before range, compare with first commit
                    const firstCommit = await this.git.raw(['rev-list', '--max-parents=0', 'HEAD']);
                    diffArgs = [`${firstCommit.trim().split('\n')[0]}..HEAD`];
                }
            } catch (error) {
                // Fallback to showing all changes in current branch
                console.log('Warning: Could not find base commit for time range, showing recent changes');
                diffArgs = ['HEAD~10..HEAD']; // Show last 10 commits as fallback
            }
        } else {
            // Note: Git diff uses target...source to show "what's in source but not in target"
            diffArgs = [`${targetBranch}...${sourceBranch}`];
        }
        ...
        for (const file of filteredFiles) {
            try {
                // Use the same diffArgs base for consistency
                const fileDiffArgs = [...diffArgs.slice(0, 1), '--', file.path];
                
                // ファイルごとの差分を取得
                file.diff = await this.git.diff(fileDiffArgs);
