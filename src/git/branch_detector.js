    ...
    constructor() {
        this.patterns = {
            production: [
                ...
                { pattern: /^(origin\/)?main-main$/, priority: 1 },  // Support main-main pattern
                { pattern: /^(origin\/)?master$/, priority: 2 },
                { pattern: /^(origin\/)?master-master$/, priority: 2 },  // Support master-master pattern
                ...
                { pattern: /^(origin\/)?stable$/, priority: 7 }
            ],
            staging: [
                { pattern: /^(origin\/)?staging$/, priority: 1 },
                { pattern: /^(origin\/)?staging-staging$/, priority: 1 },  // Support staging-staging pattern
                ...
                { pattern: /^(origin\/)?pre-production$/, priority: 9 }
            ],
            development: [
                { pattern: /^(origin\/)?develop$/, priority: 1 },
                { pattern: /^(origin\/)?develop-develop$/, priority: 1 },  // Support develop-develop pattern
                { pattern: /^(origin\/)?development$/, priority: 2 },
                { pattern: /^(origin\/)?dev$/, priority: 3 },
                { pattern: /^(origin\/)?dev-dev$/, priority: 3 },  // Support dev-dev pattern
                { pattern: /^(origin\/)?next$/, priority: 4 },
                { pattern: /^(origin\/)?integration$/, priority: 5 },
                { pattern: /^(origin\/)?feature$/, priority: 6 }
            ]
        };
        
        // Add special patterns for duplicate naming conventions
        this.duplicatePatterns = [
            /^(origin\/)?(.+)-\2$/  // Matches any pattern like xxx-xxx (e.g., main-main, dev-dev)
        ];
    }
