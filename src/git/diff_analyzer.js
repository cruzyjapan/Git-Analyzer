...
export class DiffAnalyzer {
    ...
    async analyzeBranchDiff(sourceBranch, targetBranch, options = {}) {
        ...
        // Handle special case where source and target are the same
        // This is useful for analyzing changes over time in the same branch
        const isSameBranch = sourceBranch === targetBranch || 
                            sourceBranch === `${targetBranch}-${targetBranch}` ||
                            targetBranch === `${sourceBranch}-${sourceBranch}`;
        
        // Normalize branch names for duplicate patterns (e.g., main-main -> main)
        ...
        if (isSameBranch || normalizedSource === normalizedTarget) {
            scopeMessage += ` within ${normalizedSource} branch`;
            if (!filters.since && !filters.fromCommit) {
                // Set default time range for same branch comparison
                filters.since = filters.since || '1.week.ago';
                console.log(chalk.yellow('ℹ️  Same branch detected. Using default time range: 1 week'));
            }
        } else {
            scopeMessage += ` between ${normalizedSource} and ${normalizedTarget}`;
        }
    ...
    normalizeBranchName(branchName) {
        // Handle duplicate patterns like main-main, dev-dev, etc.
        const duplicateMatch = branchName.match(/^(origin\/)?(.+)-\2$/);
        if (duplicateMatch) {
            return duplicateMatch[1] ? `${duplicateMatch[1]}${duplicateMatch[2]}` : duplicateMatch[2];
        }
        return branchName;
    }
